...
# 不会触发 1-ULP 抖动，可以对 d_ki 逐元素比较。
SPARSE_GRAD_CANN_TRITON_STRICT_CHECK_TEST_CONFIGS = [
    (1, 1, 2048, 32, 512, 8, 128, 1024),
    (1, 4, 2048, 64, 512, 16, 128, 1024),
]

# 宽松校验用 shape（满足以下任一条件时归入本组，跳过 d_ki 元素级比较）：
#   1. Nidx1 ≥ 32 或 topK ≥ 2048 → d_ki 走 atomic_add 累加深度大，
#      不同硬件/驱动/aicore 调度会产生 1-ULP 元素级抖动
#   2. S1 ≥ 512 → d_qi 分块累加路径长，fp16/bf16 舍入误差累积
SPARSE_GRAD_CANN_TRITON_LOOSE_CHECK_TEST_CONFIGS = [
    (1, 4, 2048, 128, 512, 32, 128, 1024),  # Nidx1=32
    (1, 4, 4096, 32, 512, 64, 128, 2048),   # Nidx1=64, topK=2048
...
# 严格校验用 shape：Nidx1 ≤ 16 且 topK ≤ 1024，d_ki 累加深度小，可逐元素比较。
...
# 宽松校验用 shape（满足以下任一条件时归入本组，跳过 d_ki 元素级比较）：
#   1. Nidx1 ≥ 32 或 topK ≥ 2048 → d_ki atomic_add 累加顺序在 CANN 与 numpy
#      之间不一致，必然产生 1-ULP 抖动
#   2. S1 ≥ 512 → d_qi 分块累加路径长，fp16/bf16 舍入误差累积
SPARSE_GRAD_CANN_NUMPY_LOOSE_CHECK_TEST_CONFIGS = [
    (1, 4, 2048, 128, 512, 32, 128, 1024),  # Nidx1=32
    (1, 4, 2048, 64, 512, 64, 128, 2048),   # Nidx1=64, topK=2048
    (1, 4, 4096, 32, 512, 64, 128, 2048),   # Nidx1=64, topK=2048
]
