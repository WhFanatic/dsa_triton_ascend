...
def _assert_allclose_with_values(name, expected_value, actual_value,
    ...
    expected_np = _as_numpy(expected_value)
    actual_np = _as_numpy(actual_value)
    try:
        np.testing.assert_allclose(expected_np, actual_np, atol=ATOL, rtol=RTOL)
...
@pytest.mark.parametrize("B,S1,S2,N1,N2,D,Nidx1,D_idx", DENSE_LSE_TEST_CONFIGS)
def test_dense_softmax_lse_precision(B, S1, S2, N1, N2, D, Nidx1, D_idx):
    from dense_loss_backward_triton import DenseLightningIndexerSoftmaxLseTriton

    _log(
        f"building LSE inputs: B={B}, S1={S1}, S2={S2}, Nidx1={Nidx1}, D_idx={D_idx}"
    )
    ...
    max_index_tri, sum_index_tri = stats_cell(qi, ki, w)
...
@pytest.mark.parametrize("B,S1,S2,N1,N2,D,Nidx1,D_idx", DENSE_LSE_TEST_CONFIGS)
def test_dense_grad_kl_loss_triton_supported_shapes(B, S1, S2, N1, N2, D, Nidx1, D_idx):
...
@pytest.mark.skipif(
    not _env_flag("DENSE_CHECK_CANN_LSE"),
    reason="set DENSE_CHECK_CANN_LSE=1 to compare NumPy LSE with CANN LSE",
)
...
if __name__ == "__main__":
    for config in DENSE_TEST_CONFIGS:
        test_dense_grad_kl_loss_precision(*config)
    print("dense grad kl loss test passed!")
