...
def _select_block_config(D, N1):
    ...
    # at 64 the tl.trans(k_cat) tile ([D_TOT,BK]=72KB) overflows UB. BK_B capped
...
def _sfa_grad_kernel(
    ...
    removes the per-iteration blk_in_count. (tok!=-1 is i32 NE,
    not lowered.) dt_lt_D (rope-col padding) is hoisted to a one-time compute.
    ...
    for hc in range(HC_LOOP):
        ...
        g_valid = g_offs < N1
        # 不 clamp: g_valid 依赖的地址运算(tl.where)会让 BiShengHIR 对 N1 specialize,
        # head tile 缩成 size-1 致 N1=1 编译失败; 越界头靠 mask=g_valid 屏蔽, Ascend masked 越界访问安全。
        g_offs_s = g_offs
    ...
    # NO k_cat gather, NO scores/P/dS recompute. dk = dS^T · q_cat.
    # dk_acc persists across hc iterations within one blk_start; single
    # atomic_add per output after all hc.
    # ──────────────────────────────────────────────────────────────
    ...
    # recompute. dv = P^T · do. Separate from B1 to reduce peak UB (dk_acc and
    # dv_acc coexist would overflow).
    # ──────────────────────────────────────────────────────────────
    for blk_start in range(0, topK, BLOCK_K_B):
        blk_offs = blk_start + blk_k_offs_b
        if NEED_BLK_MASK_B:
