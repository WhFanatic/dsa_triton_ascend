...
def _sfa_grad_kernel(
    ...
    do_ptr, o_ptr,                       # d_out/out [B*S1, N1, D] (nope dim only)
    ...
    dkcat (dS·q_cat) and dv (P·dO) into fp32 workspaces.
    ...
    for hc in range(HC_LOOP):
        ...
        g_valid = g_offs < N1
        # g_offs_s = g_offs directly (no clamp): out-of-range heads (g_offs >=
        # N1) carry out-of-buf addresses but are masked away by g_valid in
        # load/store. Any g_valid-dependent address op (tl.where or arithmetic)
        # makes BiShengHIR specialize N1 and shrink the head tile to size-1,
        # failing compile. Masked out-of-buf access on Ascend is safe (N1=1).
        g_offs_s = g_offs

        # Load per-head-chunk inputs. q is nope|rope concatenated on host. do/o are
        # nope-only (D); load them padded to D_TOT with zeros (mask dt<D) so dPv can
        # dot against full k_cat (rope cols contribute 0) — triton-ascend rejects
        # column slicing, so we pad-and-dot instead of slicing k_cat.
        ...
        o_pad = tl.load(o_ptr + dq_row_base + g_offs_s[:, None] * D + dt_offs[None, :],
                        mask=g_valid[:, None] & (dt_offs[None, :] < D), other=0.0)
    ...
    for blk_start in range(0, topK, BLOCK_K):
        ...
        # Load k_cat once per blk_start (shared across all hc)
        k_cat = tl.load(
            kcat_ptr + kcat_base + tok_clamped[:, None] * D_TOT + dt_offs[None, :],
            mask=tok_valid[:, None], other=0.0)

        dkcat_acc = tl.zeros([BLOCK_K, D_TOT], dtype=tl.float32)
